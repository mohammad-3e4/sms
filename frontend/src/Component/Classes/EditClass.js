...
const [rotate, setRotate] = useState(false);
  ...
  const { error, message, loading, classes } = useSelector((state) => state.classes);
  const handleRefresh = () => {
    setRotate(true);
    setTimeout(() => {
      setRotate(false);
    ...
    <>
      ...
      <div className="p-3 bg-white w-full ">
        <div className="modal-body flex flex-col lg:flex-row">
          ...
          <div className="w-3/4 text-gray-600 w-full">
            {selectedClass && (
              <table className="w-full text-center">
                <thead>
                  <tr className="border bg-[#eab308] text-white">
                    <th className="w-1/2 px-4 py-2 border-r-2 border-gray-500  text-center">Assigned</th>
                    <th className="w-1/2 px-4 py-2 text-center">Unassigned</th>
                  </tr>
                </thead>
                <tbody>
                  {classes
                    ...
                    .map((classData) =>
                      Object.keys(classData)
                        ...
                        ?.map((subject) => (
                          <tr key={subject} className="border border-gray-200 py-12">
                            <td className="px-32 py-2 border-r-2 border-gray-800">
                              {classData[subject] === "yes" && (
                                <div
                                  className="flex justify-between items-center cursor-pointer"
                                  onClick={() => handleRemoveClick(classData.class_name, subject,"rem")}
                                >
                                  <MdCancel className="h-4 w-4 mr-2 text-red-500" aria-hidden="true" />
                                  {subject}
                                </div>
                              )}
                            </td>
                            <td className="px-32 py-1 ">
                              {classData[subject] === "no" && (
                                <div
                                className="flex justify-between items-center cursor-pointer"
                                onClick={() => handleRemoveClick(classData.class_name, subject ,"add")}
                              >
                                <FaCheckCircle className="h-4 w-4 mr-2 text-green-500" aria-hidden="true" />
                                {subject}
                              </div>
                              )}
                            </td>
